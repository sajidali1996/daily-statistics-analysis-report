...
analyze_alert_codes_v2_table(T,time)
%%

function alert_table = analyze_alert_codes_v2_table(T, time)
    % Initialize previous alert code to an invalid value (e.g., 0)
    previous_alert_code = 0;

    % Initialize the output table
    alert_table = table([], [], [], [], 'VariableNames', {'Time', 'AlertCode', 'AlertName', 'AlertDescription'});
    ...
    if any(T.Alerts_2 > 0)
        ...
        for i = 1:length(Alert_2)
            ...
            if current_alert_code ~= previous_alert_code
                ...
                for j = 1:length(alerts)
                    ...
                    alert_table = [alert_table; new_row];
                end
            end
        ...
        fprintf("There were no alerts in the dataset.\n");
    end
